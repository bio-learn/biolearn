...
def estimate_sex(dnam_data):
    ...
    reference = pd.read_csv(get_data_file("estimateSex.csv"), index_col=0, low_memory=False)
    ...
    for chr in ['X', 'Y']:
        ...
        pred = (z_data.loc[chr_ref.index].T - chr_ref['mean'].values).dot(chr_ref['coeff'].values)
        pred_xy[chr] = pred
