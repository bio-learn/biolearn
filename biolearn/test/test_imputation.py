...
def test_impute_from_standard_specific_cpgs():
    ...
    df_filled = impute_from_standard(df_test, cpg_averages_test, cpgs_to_impute=specific_cpgs)

    print(df_filled)
    print(df_test)
    assert not df_filled.loc[specific_cpgs].isna().any().any()
    assert df_filled.drop(specific_cpgs).isna().sum().sum() == df_test.drop(specific_cpgs).isna().sum().sum()
