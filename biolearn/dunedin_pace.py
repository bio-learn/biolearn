...
def dunedin_pace_normalization(dataframe):
    ...
    dataframe_transposed = dunedin_pace_preprocess_data(dataframe.transpose(), gold_standard_df)
    
    # Keep only rows present in gold_standard_means
    dataframe_transposed = dataframe_transposed[dataframe_transposed.index.isin(gold_standard_means)]
    
    # Ensure target is ordered according to dataframe_transposed's rows
    target_ordered = dataframe_transposed.index.map(gold_standard_means).tolist()

    dataframe_normalized = quantile_normalize_using_target(dataframe_transposed.values, target_ordered)
    dataframe_normalized = pd.DataFrame(dataframe_normalized, columns=original_index, index=dataframe_transposed.index)
...
def quantile_normalize_using_target(data, target_values):
    ...
    for _, column_data in enumerate(data.T):
        ...
        column_data[has_decimal_above_0_4] = 0.5 * (sorted_target[rank_floor_values[has_decimal_above_0_4] - 1] 
                                                   + sorted_target[rank_floor_values[has_decimal_above_0_4]])
        column_data[~has_decimal_above_0_4] = sorted_target[rank_floor_values[~has_decimal_above_0_4] - 1]
...
def dunedin_pace_preprocess_data(betas, means,  proportionOfProbesRequired=0.8):
    ...
    probeOverlap = len(set(betas.index) & set(model_probes)) / len(set(model_probes))
    probeOverlap_background = len(set(betas.index) & set(gold_standard_probes)) / len(set(gold_standard_probes))

    if probeOverlap < proportionOfProbesRequired or probeOverlap_background < proportionOfProbesRequired:
        raise(Exception("Number of datapoints does not meet minimum required for this clock"))

    betas_mat = betas.loc[gold_standard_probes].copy()

    probesNotInMatrix = [probe for probe in gold_standard_probes if probe not in betas_mat.index]

    for probe in probesNotInMatrix:
        betas_mat.loc[probe] = gold_standard_means_lookup[probe]

    samplesToRemove = betas_mat.columns[(betas_mat.notna().sum(axis=0) / len(betas_mat)).lt(proportionOfProbesRequired)]

    betas_mat.drop(columns=samplesToRemove, inplace=True)

    pctValuesPresent = betas_mat.notna().mean(axis=1)

    probesToAdjust = pctValuesPresent.index[(pctValuesPresent < 1) & (pctValuesPresent >= proportionOfProbesRequired)]
    for probe in probesToAdjust:
        betas_mat.loc[probe].fillna(betas_mat.loc[probe].mean(), inplace=True)

    probesToReplaceWithMean = pctValuesPresent.index[pctValuesPresent < proportionOfProbesRequired]
